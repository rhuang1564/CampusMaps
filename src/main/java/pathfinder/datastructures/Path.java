...
public class Path<T> implements Iterable<Path<T>.Segment> {
  ...
  // AF(this) =
  //      first node with type T in the path => start
  //      each "step" along the path between nodes with types T => elements of list path, where
  //          path.get(0) is the first step from the start node with type T to an intermediate node, and
  //          path.get(path.size() - 1) is the final step from an intermediate node with type T to the end
  //      total cost along the path => cost
  //      the destination node with type T in this path, opposite the start node with type T => getEnd()
  ...
  /**
   * The node with type T at the beginning of this path.
   */
  private T start;
   ...
   * from the start node with type T to itself with a total cost of "0".
   * @param start The starting node with type T of the path.
   * @spec.requires start to be immutable or never to be modified
   */
  public Path(T start) {
    this.start = start;
    this.cost = 0;
   ...
   * segmentCost}.
   *
   * @param newEnd      The node with type T being added at the end of the segment being appended to this path
  ...
  /**
   * @return The node with type T at the beginning of this path.
   */
  public T getStart() {
    return start;
  }
  ...
  /**
   * @return The node with type T at the end of this path, which may be the start node with type T if this path
   * contains no segments (i.e. this path is from the start node with type T to itself).
   */
  public T getEnd() {
    if(path.size() == 0) {
      return start;
   ...
   * node with type T and ending at the end node. In the case that this path represents a path between
   * the start node with type T and itself, this iterator contains no elements. This iterator does not
   ...
   * both paths are empty, they are only equal if their starting node with type T is equal.
   *
  ...
  public class Segment {
     ...
     *
     * @param start The starting node with type T of this segment.
     * @param end   The ending node with type T of this segment.
     * @param cost  The cost of travelling this segment.
     * @throws NullPointerException     if either node with type T is null.
    ...
    /**
     * @return The beginning node with type T of this segment.
     */
    public T getStart() {
    ...
    /**
     * @return The ending node with type T of this segment.
     */
    public T getEnd() {
      return this.end;
    }
